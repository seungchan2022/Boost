...
extension MovieHomePage {
  ...
  private var itemListComponentViewState: ItemListComponent.ViewState {
    .init(itemList: [
      ItemListComponent.ViewState.MovieItem(
        title: "메가로돈 2",
        voteAverage: 0.70,
        releaseDate: "8/2/23",
  ...
  private var searchResultMoviesComponenetViewState: SearchResultMoviesComponenet.ViewState {
    //    .init(text: "SearchResultMoviesComponenet")
    .init(
      itemList: [
    ...
    VStack {
      ...
      if viewStore.keyword.isEmpty {
        ...
        Group {
          ...
          case .movies:
            // 서치 했을때의 무비 리스트
            SearchResultMoviesComponenet(
              viewState: searchResultMoviesComponenetViewState)
